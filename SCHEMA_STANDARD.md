...
> 4파트(필드/퀘스트/레벨/전투) 개발 파이프라인 xlsx/csv/JSON 표준 스키마. 웹앱 DB와 운영 시트의 단일 진실 원천. **v1.3 (2026-04-24): Phase 5 Part 2 UI 완료 (task.version_id 4뷰 전환)**
...
## 13. Task-level version_id (v1.2, Phase 5)

### 13-1. 동기

xlsx에 같은 콘텐츠명이 여러 행에 걸쳐 단계별/빌드별로 분할 등재되는 케이스(예: 한계 돌파 던전 6행, 4.2 서브퀘스트 9행) 대응. 이전에는 2번째 행부터 `::N` suffix로 별도 content를 생성했으나, 실무상 같은 콘텐츠의 버전별 분할임.

### 13-2. 스키마 변경 (v13)

- `tasks.version_id TEXT REFERENCES versions(id)` 추가 (nullable)
- 인덱스 `idx_tasks_version_id`, `idx_tasks_content_version`
- NULL이면 `contents.version_id` 상속으로 해석

### 13-3. 정책: content.version_id

- **기본 버전(첫 행)으로 축소**. 필터는 task 기준으로 전환.
- multi-value 라벨(예: 'multi') 도입하지 않음.

### 13-4. 파서 규칙 (v1.2)

| 조건 | 동작 |
|---|---|
| 같은 콘텐츠명 반복 + 게이트/분류/이슈타입 empty | Sub-Task (Level-style, 기존) |
| 같은 콘텐츠명 반복 + 게이트/분류/이슈타입 채워짐 | **root content 재사용, task.version_id = 행의 목표** |
| "ㄴ " 접두사 | Sub-Task (기존) |
...
| 2026-04-24 | 1.2 | Task-level version_id 도입 (v13). content.version_id 는 기본 버전으로 축소, 필터는 task 기준. ::N suffix 콘텐츠 병합 (28 base 그룹, 76 분할 건). |
| 2026-04-24 | 1.3 | Phase 5 Part 2 UI 완료. BuildView (task.version_id 기준 버킷 + 🔀 multi-version 배지), TaskSheet (version 컬럼/필터/편집 드롭다운), GanttView (version 필터 + task bar 버전 배지), PipelineView (태스크 행 version 셀렉트). 콘텐츠/태스크 버전 불일치 시 앰버 하이라이트. |
